...
resource "aws_backup_vault" "c1_prod_apne1_backup_vault" {
  name = "c1_prod_apne1_backup_vault"
  tags = {
...
resource "aws_backup_plan" "c1_prod_apne1_backup_plan" {
  name = "c1_prod_apne1_backup_plan"

  rule {
    rule_name         = "c1_ec2_backup_${local.backups.retention}_day_retention"
    target_vault_name = aws_backup_vault.c1_prod_apne1_backup_vault.name
...
resource "aws_backup_selection" "c1_server_backup_selection" {
  ...
  name         = "c1_prod_apne1_server_resources"
  plan_id      = aws_backup_plan.c1_prod_apne1_backup_plan.id
