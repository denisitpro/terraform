resource "aws_instance" "mgmt_c1_prod_euc1_admins" {
  count         = 1
  ...
  private_ip         = var.c1_prod_euc1_mgmt_ip

  lifecycle {
    ignore_changes = [user_data, ami, key_name]
  }
  metadata_options {
  ...
  tags = {
    Name         = "${var.c1_region_code}-${var.c1_stand}-mgmt-${format("%02d", count.index + 40)}"
